...
What is the downside to using point estimates? Hard to get accurate with 1 guess.
Tradeoff btwn accuracy and precision. 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-alicechun/manhattan.csv")
```
...
What does this mean?
We're 95% confident that between the confidence interval contains the true parameter.
...
What does this mean?
We are 95% confident that the true proportion of mice that died while not on
...
We can be 90% confident that the proportion of mice that survive while on
radiation treatment is captured by the interval (0.54, 0.66).
